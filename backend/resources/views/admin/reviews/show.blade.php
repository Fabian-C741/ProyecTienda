...
<div class="mb-6">
    <h2 class="text-xl font-semibold mb-2">{{ $productReview->product->name }}</h2>
    <div class="flex items-center gap-4 text-sm text-gray-600">
        <span>Por: <strong>{{ $productReview->user->name }}</strong></span>
        <span>•</span>
        <span>{{ $productReview->created_at->format('d/m/Y H:i') }}</span>
    </div>
</div>

<div class="mb-6">
    <div class="text-2xl text-yellow-500 mb-2">
        {{ str_repeat('⭐', $productReview->rating) }}
    </div>
    <span class="text-gray-600">{{ $productReview->rating }}/5</span>
</div>

<div class="mb-6">
    <h3 class="font-semibold mb-2">Comentario:</h3>
    <p class="text-gray-700 bg-gray-50 p-4 rounded-lg">{{ $productReview->comment }}</p>
        ...
        <form action="{{ route('admin.reviews.destroy', $productReview) }}" method="POST" class="inline">
            @csrf
            @method('DELETE')
